...
public final class SoapClient {
    ...
    public HttpResponse postRequest(String requestEnvelope) {
        return postRequest(NULL_SOAP_ACTION, requestEnvelope);
    }
     ...
     * @return The result returned by the SOAP server
     */
    public HttpResponse postRequest(String soapAction, String requestEnvelope) {
        log.debug(String.format("Sending request to host=[%s] action=[%s] request:%n%s", endpointUri.toString(),
                soapAction, requestEnvelope));
        HttpResponse response = executePostRequest(generatePost(soapAction, requestEnvelope));
        log.debug("Received response:\n" + requestEnvelope);
        return response;
    }
    
    public HttpResponse executePostRequest(HttpPost post) {
        try {
            HttpResponse response = client.execute(post);
            return response;
        } catch (SoapException ex) {
