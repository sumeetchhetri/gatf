        ...
        plot.insertSeriesDataPointsCanvasText = function (ctx) {
            ...
            for (var i = 0; i < seriesOption.length; ++i) {
                    ...
                    if (plot.getData()[contextObj.series]) {
                        ...
                        var seriesLength = seriesData.length;   // as each seriesData[j] object counts twice (x,y)                      
                        for (var j = 0; j < seriesLength; ++j) {
                             ...
                             x = plot.getAxes().xaxis.p2c(j)  + margin + resultObj.leftOffset;							 
                             y = o.top + margin + resultObj.topOffset;                      
                             y += ctx.fontAscent();
							 if(j==seriesLength-1)
							 {
								 x -= (CanvasTextFunctions.measure(resultObj.label) + 10);
								 y += 18;
							 }
