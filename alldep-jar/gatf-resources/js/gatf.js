...
function showTable(reportType, type, perfType)
{
	...
	$.each(reports, function(i, item) {
		...
		$(tr).html(
		"<td>" + identi + "</td><td><p><a href=\"#\" onclick=\"replayTest('"+trid+"',"+i+")\">Replay</a></p>"+reports[i].method+"</td><td><span class=\""+glyph+"\"></span>"+reasonCd+"</td><td>" + execTimedis + "</td><td data-id=\"tddata-"+i+"\" data-hid=\"tddatah-"+i+"\" style=\"cursor:pointer;cursor:hand;\" onclick=\"openTestCaseDetails(event, this);return false;\">"+content+"</td>").appendTo('#dataTables-example');
		$(trid).data('tcf', tcf);
		$(trid).data('tc', tc);
...
{	
	...
	if(isShowOthers)
	{
		...
		if((userSimulation && value=='All') || compareEnabled)
		{
			if(multlinegrpwid)
				$("#flot-line-chart").width(multlinegrpwid);
			showMultiLineGraph();
...
function saveSingleMultiLineGraphs()
{
	...
	if((userSimulation && currv=='All') || compareEnabled)
	{
