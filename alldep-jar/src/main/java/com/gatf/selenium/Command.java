...
public class Command {
    ...
    static Command handleActions(String cmd, FindCommand fcmd, Object[] cmdDetails, CommandState state) {
        ...
        } else if (cmd.toLowerCase().startsWith("typenb ")) {
            comd = new TypeCommand(cmd.substring(7), cmdDetails, state, true);
    ...
    public static class ExecCommand extends Command {
        ...
        String javacode() {
            ...
            code = code.replace("@line", state.currvarname()!=null?state.currvarname():"@line");
    ...
    public static class SubTestCommand extends Command {
        ...
        String javacodeint() {
            ...
            if(!children.isEmpty())
            {
            	b.append(genDebugInfo(this));
            	if(!isAnAlias) {
            		b.append("set__subtestname__(\""+esc(name)+"\");");
            	}
                b.append("\ntry {\n");
                for (Command c : children) {
                ...
                b.append("\n}\nfinally {\n");
                if(!isAnAlias) {
                	b.append("set__subtestname__(null);");
                }
                b.append("\n}");
            }
        ...
        WaitTillElementVisibleOrInvisibleCommand(String time, String val, Object[] cmdDetails, CommandState state, boolean isVisible, boolean forced) {
            ...
            if(parts.length>1) {
                if (parts[1].toLowerCase().equals("type") || parts[1].toLowerCase().equals("typenb") 
                		|| parts[1].toLowerCase().equals("sendkeys") || parts[1].toLowerCase().equals("select") 
                        ...
                        || parts[1].toLowerCase().equals("chordnb") || parts[1].toLowerCase().equals("sendkeysnb")
                        || parts[1].toLowerCase().equals("randomizenb") || parts[1].toLowerCase().equals("upload")) {
                    cmd = "";
                    ...
                    cmd = cmd.trim();
                    if(!cmd.isEmpty()) {
                        if (cmd.toLowerCase().startsWith("type ") || parts[1].toLowerCase().startsWith("sendkeys ") 
                        		|| cmd.toLowerCase().startsWith("select ") || cmd.toLowerCase().startsWith("typenb ")
                                ...
                                || cmd.toLowerCase().startsWith("chordnb ") || cmd.toLowerCase().startsWith("randomizenb ")
                                || cmd.toLowerCase().startsWith("sendkeysnb ") || cmd.toLowerCase().startsWith("upload ")) {
                            //cmd = unsanitize(cmd);
        ...
        ValidateCommand(String time, String val, Object[] lineNumber, CommandState state) {
            ...
            if(parts.length>1) {
                if (parts[1].toLowerCase().equals("type") || parts[1].toLowerCase().equals("typenb") 
                        ...
                        || parts[1].toLowerCase().equals("chord") || parts[1].toLowerCase().equals("randomize")
                        || parts[1].toLowerCase().equals("chordnb") || parts[1].toLowerCase().equals("sendkeysnb")
                        || parts[1].toLowerCase().equals("randomizenb")) {
                    cmd = "";
                    for (int i = 1; i < parts.length; i++)
                    {
                        cmd += parts[i] + " ";
                    ...
                    if(!cmd.isEmpty()) {
                        if (cmd.toLowerCase().startsWith("type ") || parts[1].toLowerCase().startsWith("sendkeys ") 
                                ...
                                || cmd.toLowerCase().startsWith("chord ") || cmd.toLowerCase().startsWith("randomize ")
                                || cmd.toLowerCase().startsWith("chordnb ") || cmd.toLowerCase().startsWith("randomizenb ")
                                || parts[1].toLowerCase().startsWith("sendkeysnb ")) {
                            //cmd = unsanitize(cmd);
    ...
    public static class ScopedLoopCommand extends Command {
        ...
        String javacode() {
            ...
            if(!children.isEmpty())
            {
                ...
                b.append("\nObject "+vrn+"=null;\n");
                b.append("\nif(___get_var_nex__(\"@index\")!=null) "+vrn+"=(Integer)___get_var_nex__(\"@index\");\n");
                b.append("\nif("+cond.condition()+") {\n");
    ...
    public static class ProviderLoopCommand extends Command {
        ...
        String javacode() {
            ...
            else if(!children.isEmpty())
            {
                ...
                b.append("\nif(___get_var_nex__(\"@index\")!=null) "+vrn+"=(Integer)___get_var_nex__(\"@index\");\n");
                if(isStateFul) {
                	b.append("\nfor(int " + loopname + "=preStateFulProvider(\""+name+"\", 0);"+loopname+"<" + provLength + ";"+loopname+"++) {\n");
                } else {
                	b.append("\nfor(int " + loopname + "=0;"+loopname+"<" + provLength + ";"+loopname+"++) {\n");
                }
                b.append("\n___add_var__(\"@index\","+state.currvarnameitr()+");\n");
    ...
    public static class ReadFileCommand extends Command {
        ...
        String javacode() {
            ...
            if(isBinary) {
            	...
            	b.append("\nint "+loopname+"=0;");
            	b.append("\n"+state.currvarname()+"=org.apache.commons.io.FileUtils.readFileToByteArray(new java.io.File(\""+esc(name)+"\"));");
                for (Command c : children) {
                	b.append(genDebugInfo(c));
                    b.append(c.javacode());
                    b.append("\n");
            	...
            	b.append("\nString "+state.varname()+" = null;");
            	b.append("\nint "+loopname+"=0;");
	            b.append("\nwhile (("+state.currvarname()+" = "+brl+".readLine()) != null) {");
                for (Command c : children) {
                	b.append(genDebugInfo(c));
                    b.append(c.javacode());
                    b.append("\n");
    ...
    public static class TransientProviderCommand extends Command {
        ...
        TransientProviderCommand(String val, Object[] cmdDetails, CommandState state) {
            ...
            if(parts.length>=4) {
                ...
                if(parts.length>4 && parts[3].trim().equals("lazy")) {
                	isLazy = true;
                }
            } else {
                //excep
        ...
        String javacode() {
            ...
            b.append("}");
            ProviderLoopCommand plc = new ProviderLoopCommand(value, fileLineDetails, state, false, false);
                   ...
                   value = "evaluate(\"" + esc(((ValueCommand)c).value) + "\")";
                    ...
                    if(FindCommandImpl.class.isAssignableFrom(c.getClass()) && ((FindCommandImpl)c).cond==null) {
                        ...
                        if(c instanceof TypeCommand) {
                            ...
                            if(((TypeCommand)c).noblur) {
                            	action = "\"" + c.getClass().getSimpleName().toLowerCase().replace("command", "nb") + "\"";
                            }
    ...
    public static class TypeCommand extends FindCommandImpl {
        ...
        TypeCommand(String val, Object[] cmdDetails, CommandState state, boolean noblur) {
            super(cmdDetails, state);
            this.noblur = noblur;
            String[] parts = val.trim().split("[\t ]+");
            if(parts.length>0) {
        ...
        String toCmd() {
            return "type" + (!noblur?"":"nb") + " \"" + value + "\"" + (cond!=null?cond.toCmd():"");
        }
        String javacode() {
            StringBuilder b = new StringBuilder();
            if(cond!=null) {
    ...
    public static class WindowOpenSaveInterceptJsCommand extends Command {
        ...
        String javacode() {
        	if(start) return "\nwindowOpenSaveJsPre();\n";
        	else return "\nwindowOpenSaveJsPost(evaluate(\""+esc(name)+"\"), "+extractText+");\n";
        }
        public static String[] toSampleSelCmd() {
        	return new String[] {
    ...
    public static class ActionsCommand extends FindCommandImpl {
        ...
        static final String ALLCMDS = "click|clickandhold|release|dblclick|doubleclick|contextclick|clickhold|rightclick|movetoelement|moveto|keydown|keyup"
                + "|sendkeys|sendkeysnb|type|typenb|chord|chordnb|draganddrop|movebyoffset|moveby|dragdrop|";
        ...
        ActionsCommand(String val, FindCommand fcmd, Object[] cmdDetails, CommandState state) {
            ...
            while(counter<tl) {
                ...
                } else if(t[counter].toLowerCase().matches("keydown|keyup|sendkeys|type|chord|sendkeysnb|typenb|chordnb")) {
                    cmd.action = t[counter].toLowerCase();
                    if(cmd.action.equals("type") || cmd.action.equals("sendkeys") || cmd.action.equals("chord")) {
                        cmd.action = "sendKeys";
                    } else if(cmd.action.equals("typenb") || cmd.action.equals("sendkeysnb") || cmd.action.equals("chordnb")) {
                        cmd.action = "sendKeysnb";
        ...
        String _javacode(String acnm) {
            ...
            } else if(action.toLowerCase().matches("sendkeysnb") || action.toLowerCase().matches("typenb") || action.toLowerCase().matches("chordnb")) {
                String ck = state.unsanitize(expr1);
                b.append("\n"+acnm+".sendKeys(evaluate(\""+ck+"\"));");
    ...
    public static class MSwipeCommand extends FindCommandImpl {
        ...
        String javacode() {
            return "\ndoSwipe(new org.openqa.selenium.Point("+sx+", "+sy+"), new org.openqa.selenium.Point("+ex+", "+ey+"), "+d+");";
        }
    ...
    public static class MTapCommand extends FindCommandImpl {
        ...
        String javacode() {
            return "\ndoTap(null, new org.openqa.selenium.Point("+x+", "+y+"), "+d+");";
        }
        String selcode(String varnm) {
            if(varnm==null) {
                varnm = state.currvarname();
            }
            return "\ndoTap("+varnm+".get(0), null, "+d+")";
    ...
    public static class MRotateCommand extends Command {
        ...
        String javacode() {
            ...
            b.append("org.openqa.selenium.ScreenOrientation "+vn+" = ((io.appium.java_client.remote.SupportsRotation)get___d___()).getOrientation();\n");
            b.append("if("+vn+".value().equals(org.openqa.selenium.ScreenOrientation.LANDSCAPE.value())){((io.appium.java_client.remote.SupportsRotation)get___d___()).rotate(org.openqa.selenium.ScreenOrientation.PORTRAIT);}");
            b.append("else{((io.appium.java_client.remote.SupportsRotation)get___d___()).rotate(org.openqa.selenium.ScreenOrientation.LANDSCAPE);}");
            return b.toString();
        }
    ...
    public static class MHideKeyPadCommand extends Command {
        ...
        String javacode() {
            StringBuilder b = new StringBuilder();
            b.append("try{Thread.sleep(200);if(get___d___() instanceof io.appium.java_client.HidesKeyboard) ((io.appium.java_client.HidesKeyboard) get___d___()).hideKeyboard();}catch(Exception e){}");
            return b.toString();
        }
